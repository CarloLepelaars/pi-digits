...
def zeta(s: Double): Double = {
  // This is an example for positive s; you would need to find a suitable
  // algorithm for other values of s
  1 / (Gamma.gamma(s) * Math.pow(2, s) * Math.pow(Math.PI, s - 1) * sin(Math.PI * s / 2))
}
